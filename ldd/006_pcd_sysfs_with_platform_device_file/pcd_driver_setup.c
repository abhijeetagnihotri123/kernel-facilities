...
/*
Creating two variables of struct device attributes
*/

ssize_t show1(struct device *dev, struct device_attribute *attr,
			char *buf){return 0;}
ssize_t store1(struct device *dev, struct device_attribute *attr,
			 const char *buf, size_t count){return 0;}

ssize_t show2(struct device *dev, struct device_attribute *attr,
			char *buf){return 0;}


static DEVICE_ATTR(max_size , S_IRUGO | S_IWUSR , show1 , store1);

static DEVICE_ATTR(serial_number_attr , S_IRUGO , show2 , NULL);

struct attribute *pcd_attrs[] = {
    &dev_attr_max_size.attr , 
    &dev_attr_serial_number_attr.attr
};
...
int pcd_sysfs_create_files(struct device *device_pcd){
    ...
    // const struct attribute *attr1[] = {&dev_attr_max_size.attr};

    // int ret = sysfs_create_files(&device_pcd->kobj , attr1);
    // if(ret){
    //     return ret;
    // }
    // const struct attribute *attr2[] = {&dev_attr_serial_number_attr.attr};
    // ret = sysfs_create_files(&device_pcd->kobj , attr2);
    // if(ret){
    //     return ret;
    // }
